...
# Not enough diamonds
execute if score @s res_cost < cost res_cost run tellraw @a [{"text":"Not enough payment (","bold": true, "color": "dark_red"},{"score":{"name":"@s","objective":"res_cost"},"bold": true, "color": "dark_red"},{"text":"/","bold": true, "color": "dark_red"},{"score":{"name":"cost","objective":"res_cost"},"bold": true, "color": "dark_red"},{"text":" diamonds). ","bold": true, "color": "dark_red"},{"selector":"@a[gamemode=spectator,limit=1,sort=nearest]","bold": true, "color": "dark_red"},{"text":" remains dead.","bold": true, "color": "dark_red"}]


# If player has cost, tag nearest dead (spectating) player for revival 
execute if score @s res_cost >= cost res_cost run tag @a[gamemode=spectator,limit=1,sort=nearest] add res_reviving
# Take the cost from their score
execute if score @s res_cost >= cost res_cost run scoreboard players operation @s res_cost -= cost res_cost


# Return diamonds (for both branches)
execute if score @s res_cost > zero res_cost run function hc_res:return_cost
